"""
================================================================================
BÚSQUEDA WEB - VERSIÓN V7.3 - SCRAPER NATIVO (BYPASS CLOUDFLARE) 
================================================================================
"""
...
SITIOS_CUBANOS =['voypati.com', 'elyerromenu.com', 'fadiar.com', 'porlalivre.com', '1cuc.com', 'cubisima.com', 'baches.com', 'merolico.com', 'timbirichi.com', 'ringcompra.com', 'tulibreta.com', 'babalarket.com', 'ktienda.com', 'cubarush.com', 'ofertas.cu', 'revolucionycuba.com', 'compraenvio.com', 'cubashopping.com', 'bachecubano.com', 'porlamer.es', 'reventadecuba.com']
SITIOS_EXCLUIDOS =['revolico.com', 'www.revolico.com', 'revo']
SITIOS_BAJA_CALIDAD =['spanishdict.com', 'dictionary.com', 'wordreference.com', 'linguee.com', 'translate.google.com', 'deepl.com', 'reverso.net', 'wikipedia.org', 'pinterest.com/pin/', 'instagram.com/p/', 'tiktok.com/', 'facebook.com/posts', 'medium.com', 'quora.com', 'reddit.com/r/', 'amazon.com', 'play.google.com']
PALABRAS_NO_VENTA =['translate', 'translation', 'dictionary', 'definition', 'meaning', 'what is', 'how to', 'tutorial', 'guide', 'learn', 'course', 'wikipedia', 'encyclopedia', 'wiki', 'about', 'history of']
PALABRAS_VENTA =['precio', 'price', 'comprar', 'buy', 'venta', 'sale', 'sell', 'oferta', 'offer', 'descuento', 'usd', 'mlc', 'cup', 'envío', 'disponible', 'nuevo', 'usado', 'garantía', 'tienda', 'clasificado', 'anuncio', 'producto', 'modelo', 'vendo', 'se vende', 'en venta', 'whatsapp']
...
def es_resultado_de_venta(titulo: str, descripcion: str, dominio: str) -> tuple:
    ...
    for sitio_bajo in SITIOS_BAJA_CALIDAD:
        if sitio_bajo in dominio_lower: return (False, f"Sitio no comercial: {sitio_bajo}")
    for palabra in PALABRAS_NO_VENTA:
        if palabra in texto_combinado:
            ...
            if not tiene_venta: return (False, f"No es venta: '{palabra}'")
    ...
    if re.search(r'\$[\d,.]+|\d+[\d,.]*\s*(?:usd|mlc|cup|eur)', texto_combinado, re.I): return (True, "Contiene precio textual")
    if re.search(r'(se vende|en venta|for sale|vendo)', texto_combinado, re.I): return (True, "Indica venta directa")
...
def _scraper_nativo_bing(termino: str, num_resultados: int) -> List[Dict]:
    ...
    terminos_busqueda =[f"{termino} Cuba precio venta", f"{termino} comprar se vende site:voypati.com OR site:elyerromenu.com"]
                    ...
                    for li in soup.find_all('li', class_='b_algo'):
                        ...
                        if link and link not in urls_vistas:
                            ...
                            if es_venta:
                                ...
                                resultados.append({"titulo": titulo, "url": link, "descripcion": descripcion, "dominio": dominio, "fecha": "", "es_cubano": es_cubano, "prioridad": 3 if es_cubano else 2})
                    break 
                except Exception:
                    time.sleep(3)
    return resultados
            ...
            if res.returncode == 0:
                ...
                for item in data:
                    ...
                    if url and url not in urls_vistas:
                        urls_vistas.add(url)
                        todos_resultados.append({"titulo": item.get("name", ""), "url": url, "descripcion": item.get("snippet", ""), "dominio": item.get("host_name", "").lower(), "prioridad": 2})
        except Exception: pass
    if not todos_resultados: todos_resultados = _scraper_nativo_bing(termino, num_resultados)
